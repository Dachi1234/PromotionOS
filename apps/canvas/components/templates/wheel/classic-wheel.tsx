...
export function ClassicWheel({
  ...
  return (
    <div
      ...
      style={{ background: `radial-gradient(ellipse at center, ${bg}, #0a0a15)` }}
    >
      <style>{`
        @keyframes cw-glow{0%,100%{filter:drop-shadow(0 0 6px ${accent})}50%{filter:drop-shadow(0 0 18px ${accent})}}
        @keyframes cw-pulse{0%,100%{box-shadow:0 0 20px ${accent}40}50%{box-shadow:0 0 40px ${accent}90}}
          ...
          <svg viewBox={`0 0 ${VB} ${VB}`} className="block w-full h-full">
            ...
            {ticks.map((t, i) => (
              <line
                ...
                stroke={accent} strokeWidth={t.major ? 2 : 1} opacity={0.6}
              />
            ))}
            {/* Slices */}
            {slices.map((s, i) => (
              <path
                ...
                fill={s.color} stroke={accent} strokeWidth={0.5}
                style={{
          ...
          >
            <span
              style={{
                ...
                color: accent,
                letterSpacing: 2,
                textShadow: `0 0 20px ${accent}, 0 0 40px ${accent}60`,
              }}
            >
              {result}
            </span>
      ...
      <button
        ...
        style={{
          ...
          background: canSpin
            ? `linear-gradient(145deg, #f5e6a3, ${spinButtonColor || accent}, #8B6914)`
            : '#444',
          color: canSpin ? '#1a1a2e' : '#888',
          fontFamily: 'Georgia, serif',
          fontSize: wheelSize * 0.032,
          border: `3px solid ${canSpin ? accent : '#555'}`,
      ...
      {spinsRemaining !== null && spinsRemaining > 0 && (
        <div className="flex gap-1.5">
          {Array.from({ length: Math.min(spinsRemaining, 10) }, (_, i) => (
            <div
              ...
              style={{
                ...
                background: `linear-gradient(145deg, ${accent}, #8B6914)`,
                color: bg,
          ...
          {spinsRemaining > 10 && (
            <span style={{ color: accent, fontSize: 12, alignSelf: 'center' }}>
              +{spinsRemaining - 10}
            </span>
          )}
        </div>
