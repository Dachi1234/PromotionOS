...
export function ClassicCTA({
  ...
  if (optedIn) {
    return (
      <div className="w-full max-w-xs mx-auto" style={{ color: textColor }}>
        <motion.div
  ...
  return (
    <div className="w-full max-w-xs mx-auto">
      ...
      <motion.button
        ...
        style={{
          background: eligible
            ? `linear-gradient(135deg, ${accentColor}, ${secondary})` : '#d1d5db',
          color: eligible ? textColor : '#9ca3af',
          cursor: eligible ? 'pointer' : 'not-allowed',
          boxShadow: eligible
            ? `0 4px 0 ${darken(secondary, 30)}, 0 6px 16px ${accentColor}30`
            : '0 2px 0 #b0b0b0',
          transform: 'translateY(0)',
        }}
        whileHover={eligible && !reduced ? {
          y: -2,
          boxShadow: `0 6px 0 ${darken(secondary, 30)}, 0 10px 24px ${accentColor}40`,
        } : {}}
        whileTap={eligible ? {
          y: 2,
          boxShadow: `0 1px 0 ${darken(secondary, 30)}, 0 2px 8px ${accentColor}20`,
        } : {}}
      >
        {preLabel}
      </motion.button>
